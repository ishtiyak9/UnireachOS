...
definePageMeta({
  ...
  layout: "partner", // Partner Layout
  middleware: "auth",
});

useHead({
  title: "Partner Intelligence Hub | Student Profile",
});
...
// 1. Unified Profile Intelligence (Scoped to Student ID)
const {
  profile,
  pending,
  error,
...
// Derived User Data (since we are not the user)
...
const fullName = computed(() =>
  profile.value.firstName
    ...
    : user.value.email?.split("@")[0] || "Student"
);
...
const openEditDialog = (section: string) => {
  // Agents verify: prevent edits if locked
  if (isLocked.value) return;

  editingSection.value = section;

  // Initialize address types if missing
  if (
...
const saveProfile = async () => {
  // Allow Agents to save even if locked (Server enforces policy, but we removed check on server)
  // if (isLocked.value) return;

  isSaving.value = true;
  try {
    await $fetch(`/api/applicants/${userId.value}/profile`, {
      method: "PATCH",
      body: formData.value,
    });
    ...
    toast.add({
      ...
      detail: "Student profile updated successfully.",
      life: 3000,
    });
...
const submitFinalProfile = async () => {
  ...
  try {
    await $fetch(`/api/applicants/${userId.value}/profile.submit`, {
      method: "POST",
    });
    ...
    toast.add({
      ...
      detail: "Student profile is now locked for institutional review.",
      life: 5000,
    });
    refresh();
  } catch (err: any) {
...
// Unlock Request Logic (Optional for Agent - maybe they want to unlock?)
const unlockDialogVisible = ref(false);
const unlockReason = ref("");
const isRequestingUnlock = ref(false);

const submitUnlockRequest = async () => {
  // Logic mostly relevant for student self-request.
  // Maybe unnecessary for Agent if they have full edit rights.
  // But let's keep logic intact just in case.
  ...
  try {
    await $fetch(`/api/applicants/${userId.value}/unlock-request`, {
      method: "POST",
...
// Tabs
const activeTab = useState("partnerProfileTab", () => 0);
</script>
...
<template>
  <div class="relative space-y-8 pb-10">
    ...
    <div
      class="absolute -top-24 -right-24 w-96 h-96 bg-emerald-500/10 blur-[120px] rounded-full pointer-events-none"
    ></div>
    <div
    ...
    >
      <div>
        ...
        >
          <span>Partner Intelligence</span>
          <i class="pi pi-angle-right text-[10px]"></i>
          <span class="text-emerald-500 italic">Student Profile</span>
        </div>
        <h1 class="text-3xl lg:text-4xl font-black text-white tracking-tight">
          Student
          <span
            class="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-emerald-600 italic"
          >
            Profile
          </span>
        </h1>
      ...
      <div class="flex items-center gap-3">
        <Button
          ...
          class="bg-white/5! text-white! border-white/10! text-[11px]! font-black uppercase tracking-widest px-6 py-3 rounded-xl hover:bg-white/10! transition-all"
          @click="navigateTo(`/partner-portal/applicants/${userId}/documents`)"
        />
      ...
      >
        <div class="relative">
          <Avatar
            ...
            size="xlarge"
            shape="circle"
            class="bg-linear-to-br! from-emerald-500! to-emerald-700! text-black! font-black! w-24! h-24! text-3xl shadow-2xl ring-8 ring-white/[0.03] group-hover:scale-105 transition-transform duration-500"
          />
          <div
        ...
        <div class="flex-1 text-center md:text-left space-y-4">
          ...
          <div class="flex flex-wrap justify-center md:justify-start gap-6">
            ...
            >
              <i class="pi pi-id-card text-emerald-400"></i>
              <span
                >ID:
        ...
        >
          <ProfileCompletionGauge
            :user-id="userId"
            tab-state-key="partnerProfileTab"
            navigation-path=""
          />
        </div>
      </div>
          ...
          <div v-if="activeTab === 0" class="space-y-8 animate-fade-in-up">
            ...
            >
              <div class="flex justify-between items-start mb-10">
                ...
                >
                  <i class="pi pi-user text-xs text-emerald-400"></i>
                  <h3
                ...
                <Button
                  ...
                  size="small"
                  text
                  class="text-[12px]! font-black! text-surface-400! hover:text-emerald-500! uppercase tracking-widest px-4"
