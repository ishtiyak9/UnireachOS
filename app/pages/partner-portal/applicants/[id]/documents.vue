...
useHead({
  title: "Partner Intelligence Hub | Student Documents",
});
...
// Fetch Documents (Scoped to Student)
const {
  data: documents,
  refresh,
  pending: docsPending,
  error: fetchError,
} = useFetch<Document[]>(() => `/api/applicants/${userId.value}/documents`, {
  key: `documents-${userId.value}`,
  immediate: true,
});

// Fetch Profile for Education History logic
const { data: profileResponse, pending: profilePending } = useFetch<any>(
  () => `/api/applicants/${userId.value}/profile`,
  {
    key: `profile-${userId.value}`,
    immediate: true,
  }
);
...
const inferredStudyLevel = computed(() => {
  if (!educationHistory.value || !educationHistory.value.length)
    return "Bachelors";
...
// Helper to get document by category
const getDocByCategory = (cat: string) =>
  documents.value?.find(
    (d: Document) => d.category === cat && d.uploadSource === "APPLICANT"
  );

// Filter Official Docs
const officialDocs = computed(
  () =>
    documents.value?.filter((d: Document) => d.uploadSource === "STAFF") || []
);
...
const handleFileChange = async (event: Event) => {
  ...
  if (file.size > MAX_SIZE) {
    toast.add({
      ...
      detail: `"${file.name}" exceeds 500KB.`,
      life: 5000,
    });
  ...
  if (!fileExtension || !allowedExtensions.includes(fileExtension)) {
    toast.add({
      ...
      detail: `Only PDF, PNG, and JPG/JPEG are accepted.`,
      life: 5000,
    });
    if (fileInput.value) fileInput.value.value = "";
    uploadingCategory.value = null;
  ...
  fd.append("file", file);
  fd.append("category", uploadingCategory.value);

  try {
    await $fetch(`/api/applicants/${userId.value}/documents`, {
      method: "POST",
      body: fd,
    });
    toast.add({
    ...
    toast.add({
      ...
      detail: e.data?.message || "Internal Error.",
      life: 5000,
    });
  } finally {
    uploadingCategory.value = null;
...
const submitDocUnlockRequest = async () => {
  if (unlockReason.value.length < 10) {
    toast.add({
      ...
      detail: "Please provide min 10 chars.",
      life: 3000,
    });
    return;
  }
  ...
  try {
    await $fetch(
      `/api/applicants/${userId.value}/documents/${docToUnlock.value}/unlock-request`,
      {
        method: "POST",
    ...
    toast.add({
      ...
      detail: "Unlock request submitted.",
      life: 5000,
    });
    ...
    toast.add({
      ...
      detail: err.data?.message || "Failed to submit.",
      life: 5000,
    });
  } finally {
...
<template>
  <div class="relative space-y-8 pb-32 max-w-5xl mx-auto">
    ...
    <!-- Background Decor -->
    <div
      class="fixed top-0 right-0 w-[500px] h-[500px] bg-emerald-500/5 blur-[120px] rounded-full pointer-events-none -mr-48 -mt-48"
    />
    ...
    >
      <div class="space-y-1">
        ...
        >
          <NuxtLink
            :to="`/partner-portal/applicants/${route.params.id}`"
            class="hover:text-emerald-500 transition-colors"
          >
            Student Profile
          </NuxtLink>
          <i class="pi pi-angle-right text-[10px]"></i>
          <span class="text-emerald-500 italic">Document Vault</span>
        </div>
        <h1 class="text-3xl lg:text-4xl font-black text-white tracking-tight">
          Document
          <span
            class="text-transparent bg-linear-to-r from-emerald-400 to-emerald-600 italic"
            >Vault</span
          >
        </h1>
        <p class="text-surface-400 text-base font-medium">
          Centralized archive for academic credentials and official
          correspondence.
        </p>
      </div>
      ...
      <div class="flex items-center gap-4">
        <Button
          ...
          text
          class="text-surface-400! hover:text-white! text-[11px]! font-black uppercase tracking-widest"
          @click="navigateTo(`/partner-portal/applicants/${route.params.id}`)"
        />
    ...
    <div class="flex items-center gap-10 border-b border-white/5 relative">
      ...
      >
        ...
        <div
          v-if="activeTab === 0"
          class="absolute bottom-0 left-0 w-full h-0.5 bg-emerald-500 shadow-[0_-4px_15px_rgba(16,185,129,0.6)]"
        />
      </button>
      <button
        @click="activeTab = 1"
    ...
    <div v-if="docsPending" class="space-y-10 animate-pulse">
      <section v-for="i in 2" :key="i" class="space-y-4">
        <div class="flex items-center gap-4">
          ...
          <div class="h-px bg-white/5 grow" />
        </div>
        <div class="flex flex-col gap-3">
          <div
            v-for="j in 3"
            :key="j"
            class="h-24 w-full bg-white/2 border border-white/5 rounded-2xl p-6 flex items-center justify-between"
          >
            <div class="flex items-center gap-4">
    ...
    >
      ...
      <section class="space-y-4">
        <h2
          class="text-[13px] font-black text-emerald-500/80 uppercase tracking-[0.4em] flex items-center gap-4"
        >
          Identity & Documents
          <div class="h-px bg-white/5 grow" />
        </h2>
    ...
    >
      <div
        class="w-16 h-16 rounded-2xl bg-emerald-500/5 flex items-center justify-center border border-emerald-500/10 group-hover:bg-emerald-500/10 transition-all duration-500"
      >
        <i class="pi pi-shield text-emerald-500 text-xl" />
      </div>
      ...
      <div class="space-y-6 py-4">
        ...
        <div class="space-y-2">
          ...
          <Textarea
            ...
            rows="4"
            autoResize
            class="w-full bg-white/3! border-white/10! text-white rounded-2xl p-4 text-sm"
