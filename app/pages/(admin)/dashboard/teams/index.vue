...
// Data Fetching
const { data: teamsRes, refresh } = await useFetch("/api/admin/teams");
const teams = computed(() => (teamsRes.value as any)?.data || []);

const { data: staffRes } = await useFetch("/api/admin/users", {
  query: { categories: ["STAFF", "SYSTEM"] },
});
const allStaff = computed(() => (staffRes.value as any) || []);

const { data: countriesRes } = await useFetch("/api/admin/countries");
